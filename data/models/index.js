...
for (const model of Object.keys(models)) {
  _.isFunction(models[model].associate) &&
    models[model].associate(models, sequelize);
  _.isFunction(models[model].addScopes) &&
    models[model].addScopes(models, sequelize);
...
models.generateSearchQuery = (string, fields = ['firstName', 'lastName']) => {
  ...
  let strings = _.take(_.words(string, /[^, ]+/g), 3);
  ...
  function generateQueryString(input) {
    for (let i = 0; i < input.length; i++) {
      ...
      if (!input.length) {
        permArr.push(
          where(columns, { [Op.iLike]: `%${usedChars.slice().join('%')}%` })
        );
      }
